    ...
    "categorias": [
        {
            ...
            "descricao": "Maior valor — destrava unidades e gera receita",
            "cor":  "#dc2626",
            "itens": [
                {"titulo": "Integração Omie (Indianópolis + São Miguel)",
                 "status": "pendente", "estimativa": "~2-3 dias",
                 "nota":   "destrava 33% da rede — 2 unidades hoje sem sistema completo. OmieClient + roteador em api_send + tela de config. Próximo grande item — janela dedicada quando Ian tiver disponibilidade."},
                {"titulo": "Cobrança — Infra contatos (pré Fase 2)",
                 "status": "pendente", "estimativa": "~3h",
                 "nota":   "pre-requisito pra Fase 2"},
                {"titulo": "Cobrança Fase 2: régua de email",
                 "status": "pendente", "estimativa": "~4-6h",
                 "nota":   "template + envio em lote"},
                {"titulo": "Cobrança Fase 3: régua WhatsApp via Z-API",
                 "status": "pendente", "estimativa": "~4-6h"},
            ],
        },
        {
            "nome": "Hardening — refactor / qualidade",
            ...
            "itens": [
                ...
                {"titulo": "Blueprints Flask por domínio",
                 "status": "pendente", "estimativa": "~2-3 dias",
                 "nota":   "server.py ~9K linhas → ~500 linhas. Caixa, Fechamento, Master, Auth viram blueprints. Refactor major mas blindado pelos 336 testes verdes."},
            ],
        },
        {
            "nome": "UX / Polimento",
            "descricao": "Experiência do operador e gerência",
            "cor":  "#10b981",
            "itens": [
                {"titulo": "Sistema de tickets/suporte",
                 "status": "pendente", "estimativa": "~1 dia"},
                {"titulo": "Unificar Gerencial Unidade + Rede em 1 tela",
                 "status": "pendente", "estimativa": "~2-3h"},
                {"titulo": "Responsividade mobile — audit cirúrgico",
                 "status": "pendente", "estimativa": "~4-6h",
                 "nota":   "quando incomodar: pedir prints das telas no celular e corrigir só o que quebra. Caixa+Fechamento críticos se cair luz numa unidade"},
                {"titulo": "Reinstalar chatbot nas páginas onde sumiu",
                 "status": "pendente", "estimativa": "~30-60 min"},
            ],
        },
        {
            "nome": "Verificar (pendências leves)",
            "descricao": "Baixo esforço, validação manual",
            "cor":  "#2563eb",
            "itens": [
                {"titulo": "Validação prod: histórico Tiny enriquecido (modelo+serviço)",
                 "status": "pendente",
                 "nota":   "Após envio real Mooca/Moema, conferir no relatório do Tiny se aparece 'Placa XXX | MARCA MODELO | SERVICO'. Já validado em código (293 testes) mas falta evidência prod."},
                {"titulo": "Validação prod: cron lock evita Backup/Caixa-do-dia duplicados",
                 "status": "pendente",
                 "nota":   "Esperar 00:00 e 18:30 chegarem só 1× cada após deploy de e557d4e"},
                {"titulo": "Dedup KAVAK/H&L",
                 "status": "pendente",
        ...
        },
        {
            ...
            "descricao": "Decisões ou ações do Ian, não automatizáveis",
            "cor":  "#0f172a",
            "itens": [
                {"titulo": "Decisão sobre mobile/tablet",
                 "status": "pendente",
                 "nota":   "nunca foi discutido explicitamente"},
                {"titulo": "Re-importar planilhas antigas pra popular modelo retroativo",
                 "status": "pendente",
                 "nota":   "Após fix do parser Sispevi (249cd8b), planilhas antigas continuam sem modelo. Re-importar pra atualizar — opcional, novos envios já pegam direto."},
            ],
        },
        {
            "nome": "Concluídos — sprint 28/04 → 01/05",
            "descricao": "Entregas dessa semana",
            "cor":  "#16a34a",
            "itens": [
                # 01/05 — feriado (manhã)
                {"titulo": "Detector de feriado nacional (suprime alerta de inatividade)",
                 "status": "concluido", "data": "2026-05-01",
                 "nota":   "Algoritmo de Meeus pra Páscoa cobre Sexta Santa, Carnaval, Corpus Christi"},
                {"titulo": "Cache pre-aquecido contas-receber (cron 4×/dia 8/12/16/20h)",
                 "status": "concluido", "data": "2026-05-01",
                 "nota":   "Resolve 224s de latência da 1ª chamada — usuário sempre vê resposta instantânea"},
                # 30/04
                {"titulo": "Email prefs + cron lock + Tiny só com falhas",
                 "status": "concluido", "data": "2026-04-30",
                 "nota":   "fcntl lock resolve duplicação Backup/Caixa-do-dia (2 workers gunicorn). Tela /master/email-prefs com 5 toggles."},
                {"titulo": "Parser Sispevi captura modelo (coluna dataIdx+1)",
                 "status": "concluido", "data": "2026-04-30",
                 "nota":   "Modelo era hardcoded como vazio no parser sem-header"},
                {"titulo": "Histórico Tiny enriquecido (Placa | MARCA MODELO | SERVIÇO)",
                 "status": "concluido", "data": "2026-04-30"},
                {"titulo": "Liberar lançamento valor R$ 0,00 no Caixa",
                 "status": "concluido", "data": "2026-04-30",
                 "nota":   "Operadoras lançavam R$ 0,01 como workaround → divergência na conferência"},
                {"titulo": "Relatório imprimível do Fechamento (PDF print-friendly)",
                 "status": "concluido", "data": "2026-04-30",
                 "nota":   "Tela /u/<unit>/fechamento/imprimir com sumário, FP detalhado, faturados, conferência PDV×planilha"},
                # 29/04
                {"titulo": "Tela /master/relatorio-mensal (Motor / Cautelar por unidade)",
                 "status": "concluido", "data": "2026-04-29",
                 "nota":   "Substitui planilha manual do Ian. Filtro mês + exportar CSV."},
                {"titulo": "Janela 90d em _fetch_contas_abertas_tiny (254s → ~30s)",
                 "status": "concluido", "data": "2026-04-29"},
                # 28/04
                {"titulo": "Smoke test pytest anti-regressão (32 endpoints)",
                 "status": "concluido", "data": "2026-04-28"},
                {"titulo": "Fix /master/api/contas-receber (paraleliza + pula não-Tiny)",
                 "status": "concluido", "data": "2026-04-28",
                 "nota":   "Era 500 em 300s (timeout gunicorn). ThreadPoolExecutor + filtra unidades sem Tiny pronto."},
                {"titulo": "Cache TTL=5s no /api/planilha/status + instrumentação timing",
                 "status": "concluido", "data": "2026-04-28"},
            ],
        },
        {
            "nome": "Concluídos — sprint 24/04 → 27/04",
            "descricao": "Histórico semana anterior",
            "cor":  "#16a34a",
            "itens": [
                # 27/04 — segunda
                {"titulo": "Migração caixa2.html → shell v2.1 (último HTML legacy)",
                 "status": "concluido", "data": "2026-04-27"},
                {"titulo": "Bug crítico do cron de tokens (chamava método inexistente — nunca rodou)",
                 "status": "concluido", "data": "2026-04-27",
                 "nota":   "endpoint manual /master/api/tokens/refresh-now criado pra emergência"},
                # 26/04 — domingo (dia massivo)
                {"titulo": "Wizard Fechamento v2 (banner alertas + decisões persistidas + cores por tipo)",
                 "status": "concluido", "data": "2026-04-26"},
                {"titulo": "Wizard v2 fases extras: Cortesia c/ PIN, Faturar, Adicionar pagamento, Marker tocado",
                 "status": "concluido", "data": "2026-04-26"},
                {"titulo": "Conferência Antecipada — planilha do dia em tempo real + click-to-fill",
                 "status": "concluido", "data": "2026-04-26"},
                {"titulo": "Hardening — Documentação (README + ARCHITECTURE.md + API.md)",
                 "status": "concluido", "data": "2026-04-26"},
                {"titulo": "Hardening — Testes 24/84 → 157/157 verde + CI GitHub Actions",
                 "status": "concluido", "data": "2026-04-26"},
                {"titulo": "Hardening — Cobertura TinyImporter + Master Unidades (+46 testes)",
                 "status": "concluido", "data": "2026-04-26"},
                # 25/04 — sábado
                {"titulo": "Dashboard de saúde consolidado em /master/sistema",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Error handler global frontend → /api/log/js-error",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Test restore B2 automatizado (mensal + botão manual)",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Renovação tokens Tiny 07h+15h SP + diag/tokens",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "CSRF retry automático em apiFetch",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Histórico Rede unificado (PDV + Emitidos Tiny + BI em tabs)",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Tela de Royalties por unidade (Itu 7%, demais 3%)",
                 "status": "concluido", "data": "2026-04-25"},
                {"titulo": "Mapear FP via UI + Definir PIN via UI + Limpar dia",
                 "status": "concluido", "data": "2026-04-25"},
                # 24/04 — sexta
                {"titulo": "Tela de gestão de unidades (criar/listar/remover)",
                 "status": "concluido", "data": "2026-04-24"},
                {"titulo": "Reorganização do menu (NAV_CATALOG por perfil)",
                 "status": "concluido", "data": "2026-04-24"},
                {"titulo": "Movido Manutenção + Backup pro /master/sistema",
                 "status": "concluido", "data": "2026-04-24"},
                {"titulo": "Backup B2 com dedup contra duplicação em restart",
                 "status": "concluido", "data": "2026-04-24"},
                {"titulo": "Gerencial Unidade — FP do dia + Top 10 clientes",
                 "status": "concluido", "data": "2026-04-24"},
                {"titulo": "Dashboard Contas a Receber alinhado com Tiny",
                 "status": "concluido", "data": "2026-04-24"},
            ],
        },
    ],
